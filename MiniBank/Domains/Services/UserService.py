class UserService(object):
    def __init__(self, application_service):
        self.application_service = application_service

    def create_user(self, username, is_person):
        uid = self.application_service.get_new_user_id()
        user = User(uid, username, is_person)
        return self.application_service.create_user(user) # returns maybe user
    
    def create_account(self, username, balance = 0):
        user = self.search_user_by_username(username) 
        if user is None:
            return None
        
        new_account = self.account_service.create_account(user.uid, balance)
        if new_account is None:
            return None

        user.add_account(new_account)

        return self.application_service.add_account(user, account)

    def get_account_summary(self, username):
        user = self.search_user_by_username(username)
        if user in None: 
            return None

        return user.get_account_summary()

    def search_user_by_username(self, username):
        return self.application_service.search_user_by_username(username)
